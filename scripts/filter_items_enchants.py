...
def is_valid_trinket_for_demon_hunter(item):
    if item['itemClass'] == 4 and item['itemSubClass'] == 0 and item['inventoryType'] == 12:
...
def is_usable_by_demon_hunter(item):
    ...
    if item['itemClass'] == 4 and item['itemSubClass'] == 1 and item['inventoryType'] == 16:
        return True
...
def is_from_specific_dungeon(item):
    ...
    if 'sources' in item:
        return any(source['instanceId'] in specific_dungeon_ids for source in item['sources'])
...
def affects_damage(enchant):
    damage_stats = ['agi', 'agility', 'stragi', 'stragiint', 'mastery', 'haste', 'crit', 'versatility', 'vers', 'damage', 'stamina']
    ...
    damage_keywords = ['damage', 'agility', 'strength', 'intellect', 'primary stat',
                       'mastery', 'haste', 'crit', 'versatility', 'stamina']
    if any(keyword.lower() in enchant['displayName'].lower() for keyword in damage_keywords):
        return True

    # Check for specific enchants that might not have clear stat indicators
    special_damage_enchants = [
        'Accelerated', 'Deadly', 'Quick', 'Accurate', 'Masterful', 'Versatile',
        'Primary Stat', 'All Stats', 'Instinctive', 'Illuminated', 'Armor Kit'
    ]
    ...
    damage_categories = ['Chest Enchantments', 'Weapon Enchantments', 'Ring Enchantments']
...
def is_usable_by_demon_hunter_enchant(enchant):
    ...
    if 'stats' in enchant:
        ...
        if 'int' in stat_types or 'intellect' in stat_types:
            if not any(stat in stat_types for stat in ['agi', 'agility', 'str', 'strength', 'stragi', 'stragiint']):
                return False
...
def filter_enchants(enchants, current_expansion):
    ...
    for enchant in enchants:
        if enchant.get('expansion') == current_expansion and is_usable_by_demon_hunter_enchant(enchant):
...
def main():
    ...
    print(f"Total flasks: {len(all_flasks)}")
    print(f"Filtered consumables for Demon Hunters: {len(all_filtered_consumables)}")
