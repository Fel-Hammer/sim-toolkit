...
def filter_demon_hunter_specs(data):
    ...
    return [
        ...
        if tree.get("className") == "Demon Hunter" and
           tree.get("classId") == 12 and
           tree.get("specName") in SPEC_NAMES
    ]
...
def fetch_talents_json(force_new=False):
    ...
    try:
        if os.path.exists(TALENTS_CACHE_FILE) and not force_new:
            cache_time = datetime.fromtimestamp(os.path.getmtime(TALENTS_CACHE_FILE))
...
def determine_hero_spec(hero_talent_string, spec_name):
    ...
    if spec_name == "vengeance":
        ...
        if "demonsurge:1" in hero_talent_string:
            ...
            return "aldrachi reaver"
    raise ValueError(f"Unable to determine HERO_SPEC for {spec_name} from hero_talent_string")
...
def generate_talent_hash(hero_talent_string, class_talent_string, spec_talent_string, spec_name, clear_cache=False, force_new=False):
    global talent_data
...
if __name__ == "__main__":
    ...
    parser.add_argument('--force-new', action='store_true', help='Force fetching a new talents.json file')
    parser.add_argument('--spec', required=True, choices=SPEC_NAMES, help='Demon Hunter specialization')
    args = parser.parse_args()

    # Initialize talent data once
    initialize_talent_data(force_new=args.force_new)

    result = generate_talent_hash(args.hero_talents, args.class_talents, args.spec_talents, args.spec, clear_cache=args.clear_cache)
    print(result)